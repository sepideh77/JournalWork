...
if __name__ == "__main__":
    ...
    """ Environment """
    env = Environment(config.Capacity_RSUs, config.Capacity_nomadic_caches, config.Cloud_transfering_delay, config.Diameter_rsus, config.G, config.Maximum_numreq_handled_CacheNode, config.Nomadic_Caches_Velocity, config.O, config.P, config.num_Nomadic_Caches, config.num_Plcd_NS_contents, config.num_Targeted_Nomadic_caches, config.num_Users_per_Nomadic_caches, config.num_descriptors, config.num_rsu, config.num_s_contents, config.size_of_contents, config.small_transfering_delay, config.transfering_delay, config.w1, config.w2, config.w3, config.w4, config.w5, config.w6)

    """ Batch of Services """
    services = ServiceBatchGenerator(config.batch_size, config.min_length, config.max_length, config.num_Plcd_NS_contents)

    """ Agent """
    state_size_sequence = config.max_length
    state_size_embeddings = config.num_descriptors     #OH Vector embedding
    action_size = config.num_Nomadic_Caches
